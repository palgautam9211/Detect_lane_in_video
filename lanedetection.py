...
while(capture.isOpened()):
    ...
    if lines is not None:
       ...
       if averaged_lines is not None:  # Ensure averaged lines are valid
           line_image = display_lines(frame, averaged_lines)
           combo_image = cv2.addWeighted(frame, 0.8, line_image, 1, 1)
           cv2.imshow("Result", combo_image)
       else:
           cv2.imshow("Result", frame)  # If no lines found, show the original frame
    else:
        cv2.imshow("Result", frame)  # If no lines detected, show the original frame
